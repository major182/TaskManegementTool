...
| ブラウザの「戻る」 | S-03 で押すと S-02 に戻る。**S-01（ログイン中）で押したときは S-01 のまま**。いずれもアプリの外へは出ない（画面ごとに履歴の目印を積み、S-01 では押されるたびに積み直す） |
...
| 2 | ボード一覧 | リンクの一覧 | 利用者が並べた順。押すと表示エリアがそのボードに切り替わる。表示中のボードは背景色で強調。**行をつかんで上下にドラッグすると並び替えられる**（5px 動かすまでは並び替えにしないので、押しただけでは動かない） | F-11、F-15、F-16 |
| 3 | ＋ ボードを作成 | ボタン | 押すと一覧の末尾に入力欄が現れる。確定で作成し、**そのボードを表示する**。新しいボードは一覧の**一番下**に入る | F-12 |
...
| 9 | 追加用の入力欄（7・8とサイドバーの「＋ ボードを作成」） | 入力欄 | **画面の関係ないところを押すと、入力途中の内容を捨てて閉じる**。作成するのは「追加」を押したときか Enter のときだけ | F-21、F-31 |
...
- 未入力のまま押したときは、通信せず「ユーザーID を入力してください」等を入力欄の近くに出す。ユーザーID は形式（半角英数字とアンダースコアで4〜20文字）も確認し、合わなければ通信しない。パスワードは未入力だけを確認する（形式で弾くと、ルール変更前に登録した人が入れなくなるため）
...
| 403（CSRF） | 「操作を完了できませんでした。ページを再読み込みしてください」＋[再読み込み] | エラー |
...
> 公開先の実行環境は、しばらくアクセスがないと停止することがあり、その場合は初回アクセスに数十秒かかります（[02 技術選定書 3.4](02_tech-stack.md#34-開発環境公開)）。読み込みが**10秒**を超えたら「サーバーの起動を待っています。しばらくお待ちください」を追加表示します。
...
## 11. 実装で決めたこと（旧「検討中・次工程で決めること」）

初版で保留にしていた4件は、フロントエンドの実装時にすべて決めました。

| No | 内容 | 決定 | 理由 |
|---|---|---|---|
| 1 | カードの説明文を、表示時に何行まで出すか | **1行に省略する**（`...` で切る） | リストが縦に長くなると、カードの枚数を見渡せなくなるため。全文はカードを押せば編集欄で読める |
| 2 | ドラッグ中の見せ方 | **つかんだカードを指先に重ねて表示し（少し傾ける）、元の位置は薄く残す**。挿入位置は他のカードがよける動きで示す（dnd-kit の標準） | 標準の見せ方に合わせることで、利用者が他のツールで慣れた操作感になるため |
| 3 | トーストの表示時間 | **エラーは手動で閉じるまで残す。お知らせは5秒で消す** | エラーを自動で消すと、何が失敗したのか分からないまま操作を続けてしまうため |
| 4 | キーボード操作だけでのドラッグ＆ドロップ | **バージョン1では対応しない** | 対応するには「どこへ移動したか」を読み上げる仕組みまで要り、バージョン1の範囲に収まらないため。並び替え以外の操作（作成・編集・削除）はキーボードだけで行える |

> No.2 の補足：ドラッグは**少し（5px 以上）動かして初めて開始**します。こうしないと、カードを押してその場編集を開く操作がドラッグとして扱われてしまうためです。
> リストの並び替えは**ヘッダーをつかんだときだけ**始まります（4.3 No.3）。カードの操作を邪魔しないようにするためです。
...
| 版数 | 日付 | 内容 | 作成者 |
|---|---|---|---|
| 0.6 | 2026-09-23 | サイドバーでボードを並び替えられること（F-16）を 4.2 に明記 | |
| 0.5 | 2026-09-23 | メイン画面（S-01）でブラウザの「戻る」を押したときの動きを 3章に明記（Issue #31 の対応） | |
| 0.4 | 2026-09-22 | 実装に合わせて修正。ログイン画面でユーザーID の形式も確認すること（5章）、403 のトーストに[再読み込み]を付けること（8.1）を明記 | |
| 0.3 | 2026-09-21 | 動作確認で見つかった点を反映。ブラウザの「戻る」の動き（3章）と、追加用の入力欄が外側のクリックで閉じること（4.3）を明記 | |
| 0.2 | 2026-09-21 | フロントエンドの実装に合わせて11章を「実装で決めたこと」に改め、保留だった4件（説明文の行数、ドラッグ中の見せ方、トーストの表示時間、キーボード操作）を決定として記載 | |
